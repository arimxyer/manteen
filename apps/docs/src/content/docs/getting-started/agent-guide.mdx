...
agent-native authoring and workflow foundation shipped in
...
package release. The complete behavior documented here is versioned for
<code>{`manteen@${manifest.releases.documentedClientRelease}`}</code>. Documentation still is not
proof of what a project has installed. Inspect `manteen --version` and command `--help`; never
invent a command or flag that the installed binary does not expose.
:::
