...
export interface InteropPublicationProps {
  reduceMotion: boolean;
  /** Zero for the settled still; incremented to play or replay the explanation. */
  run: number;
}
 ...
 */
// Measured, not estimated: head 20 + the 13px item-document label + body 117
// (five rows of ROW_H plus four 3px gaps) + foot 47 + 16 of padding + the
// separations = 225. The plate keeps 9px of headroom so a font or rounding
// difference cannot clip the last document off the bottom of the set.
const PLATE_H = 234;
const GAP = 8;
...
const TIMELINES: Record<Outcome, readonly Keyframe[]> = {
  succeeds: [
    { at: 900, phase: "rendering", filled: 1 },
    { at: 1200, filled: 2 },
    { at: 1500, filled: 3 },
    { at: 1800, filled: 4 },
    { at: 2100, filled: 5 },
    { at: 2600, filled: 6 },
    { at: 3100, filled: 7 },
    { at: 3400, phase: "complete" },
    { at: 5200, phase: "exchanging" },
    { at: 5900, phase: "published" },
  ],
  refuses: [
    { at: 900, phase: "rendering", filled: 1 },
    { at: 1250, filled: 2 },
    { at: 1750, phase: "colliding" },
    { at: 2600, phase: "refused" },
    { at: 4400, phase: "discarding" },
    { at: 5200, phase: "unchanged" },
  ],
};

const EXCHANGE_AT = 5200;
...
const CAPTIONS: Record<Phase, Caption> = {
  open: {
    lead: "A new build opens a separate output set.",
    detail: "Each appearing row is one generated registry document; the published set stays put.",
  },
  rendering: {
    lead: "The new output set fills with complete documents.",
    detail:
      "Item documents first, then the catalog index, then the marker over every emitted file.",
  },
  complete: {
...
export function InteropPublicationStudy({ reduceMotion, run }: InteropPublicationProps) {
...
/**
 * Production wrapper: one automatic explanation when half the card enters view,
 * then the outcome buttons are the replay controls. The settled still is what
 * the server and reduced-motion readers receive, so the homepage never offers a
 * transport control that cannot do anything.
 */
export function InteropPublication({ className }: { className?: string }) {
  ...
  return (
    <MotionConfig reducedMotion="user">
      <div ref={container} className={cn("flex min-w-0 flex-col justify-center", className)}>
        {allowed === true ? (
          <InteropPublicationStudy reduceMotion={false} run={run} />
        ) : (
          <div className="flex min-h-[26rem] min-w-0 flex-col justify-center">
            <StaticOutcome
              heading="Published output"
              state="complete"
              items={NEW_ITEMS}
              boundary={BOUNDARY}
              lead="A complete build becomes the published set in one move."
              detail="Every reading returns one complete output set; no reader receives the transition."
            />
...
/** Resolve the media query before the first client paint without changing the server tree. */
    ...
    <>
      <div aria-hidden="true" className="flex min-w-0 flex-col">
        ...
        >
          ...
          >
            <Plate
              head={addressed ? address : <PlateLabel>build output</PlateLabel>}
      ...
      <div className="pt-3">
        {/*
          The caption now sits directly under the output sets it explains. Its
          height remains reserved from measurement: 71px below 640, 36px from
          640 to 1023, and 53px from 1024 where the two-column grid narrows the
          card again. The non-monotonic reserve prevents phase copy from moving
          the readings or the card underneath it.
        */}
...
function stateWord(phase: Phase): string {
  if (phase === "open" || phase === "rendering") return "assembling";
...
function Plate({
  ...
  return (
    ...
    >
      <div className="hidden h-5 shrink-0 items-center justify-between gap-2 @[8rem]:flex">
        {head}
        <span
      ...
      <div className="mt-1 hidden h-2.5 shrink-0 items-center gap-2 px-0.5 @[8rem]:flex">
        <span
          className="shrink-0 font-mono text-[8px] leading-none tracking-[0.12em] uppercase"
          style={{ color: MUTED_INK }}
        >
          item documents
        </span>
        <span className="h-px min-w-2 flex-1 bg-fd-foreground/15" />
      </div>

      <div className="invisible mt-1 flex min-h-0 flex-1 flex-col gap-[3px] overflow-hidden @[8rem]:visible">
        {rendered.map((name) => (
          <Row
            key={name}
            label={`${name}.json`}
      ...
      <div className="invisible mt-1.5 flex shrink-0 flex-col gap-[3px] @[8rem]:visible">
        {refused ? (
          <Refusal />
        ) : (
          <>
