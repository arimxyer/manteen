...
| D41 | Ordinary update output remains exact, line-oriented source bytes. A ts-morph printer or `formatText()` path is rejected for merge output; a future AST pass may classify edits or supply conservative anchors only if final output preserves original byte ranges and ambiguous mappings, including delete-versus-modify, refuse. The completed 2026-08-08 classification spike does not change this production contract. | A bounded printer probe found that formatting one 119-line TSX input changed 89 lines despite changing no program behavior; every observed change was whitespace, and the tested printer path also failed the exact CRLF/BOM boundary. The later read-only classifier used exact source offsets and emitted no merge bytes. Across two real upstream pairs plus nine controlled synthetic local sides, it found one distinct-import case that line diff3 reported as a conflict and produced zero false-independent results, while refusing same-key, rename/delete, parse, default, duplicate and cross-file ambiguity. That demonstrates narrow classification value, not an output algorithm or population rate. | Making an AST printer the default merger; treating the controlled corpus as observed consumer evidence; converting `independent-candidate` directly into source output; claiming a conflict-rate improvement from one source path; weakening a refusal because an exported name merely survives a declaration-kind change. See [`update-merge-handoff.md`](update-merge-handoff.md) and the committed [`AST spike report`](ast-assisted-merge-spike.md). |
