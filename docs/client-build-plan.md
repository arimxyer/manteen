...
| `merge-base-unreadable` (version-3 receipt base missing, corrupt, or unreadable) | error | **no** | plan | 1 |
...
| D30 | Receipt v2 adds nullable `styles`; the v1 schema remains frozen. A valid v1 receipt migrates only in memory for reads and is written as v2 only after a successful mutation. | Read-only commands must not create surprise diffs, while future mutations need a truthful ownership baseline. | Mutating on read; making v1 unreadable; extending the v1 schema in place. |
...
| D34 | A default update keeps local-only changes, applies upstream-only changes, and refuses conflicts before mutation. `--take-upstream` is the only update spelling that discards local changes; update no longer accepts `--overwrite`, `--no-overwrite`, or `--yes`. | The command should mean “bring upstream forward around my adaptations.” A generic overwrite flag hides the exact destructive choice users need to understand. | Keeping the old flags as aliases — preserves the trap under familiar automation and makes `--yes` mean data loss. |
