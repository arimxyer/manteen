...
| D41 | Ordinary update output remains exact, line-oriented source bytes. A ts-morph printer or `formatText()` path is rejected for merge output; a future AST pass may classify edits or supply conservative anchors only if final output preserves original byte ranges and ambiguous mappings, including delete-versus-modify, refuse. | A bounded probe found that formatting one 119-line TSX input changed 89 lines despite changing no program behavior; every observed change was whitespace, and the tested printer path also failed the exact CRLF/BOM boundary. Returning untouched source text is exact only when the AST does not emit a replacement. The separate rename-versus-edit example was synthetic and showed a failure mode of a naive name-keyed merger, not a measured repository case: a conservative structural merger should refuse rather than discard either side. | Making an AST printer the default merger; treating one synthetic example as evidence that structural merging is broadly safe or unsafe; claiming a conflict-rate improvement from the small upstream-only history sample. See the evidence boundary in [`update-merge-handoff.md`](update-merge-handoff.md). |
