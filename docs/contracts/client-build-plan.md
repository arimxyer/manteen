...
> disagree, and D32–D41 plus [`update-merge-handoff.md`](../handoffs/update-merge-handoff.md) and
> [`update-verification-handoff.md`](../handoffs/update-verification-handoff.md) are the current receipt-v3,
...
| `receipt-invalid-json`, `receipt-schema-invalid`, `receipt-unsupported-version`, `receipt-future-version`, `receipt-io-unreadable` | warn | — | offline status | 0 |
...
| `remove-file-unowned` (a selected `--file` is not an exact ordinary destination owned by the receipt) | error | **no** | removal plan | 1 |
| `remove-file-still-published` (the selected file is still present under the same current item id) | error | **no** | removal plan | 1 |
| `remove-file-reassigned` (another current item now claims the selected destination) | error | **no** | removal plan | 1 |
| `remove-file-artifact` (the selected destination is the recorded or current theme/managed-styles artifact) | error | **no** | removal plan | 1 |
| `remove-adapted-file` (current bytes differ from the pristine `baseSha256`) | error | **no**; `--discard-adapted` changes the requested operation | removal plan | 1 |
| `remove-path-unsupported` (a selected source, base or receipt path is neither absent nor a readable regular file, including lstat/read failure) | error | **no** | removal plan + preflight | 1 |
...
| D39 | After a successful non-preview `add`, `update`, or D42 upstream-removal transaction actually changes the receipt or a pristine base, apply reports that observed state mutation and the text CLI emits one `state-versioning-required` advisory at `info`. Update/removal JSON carries the same fact structurally. Manteen does not inspect Git or claim the state is tracked. | Receipt v3 and `.manteen/bases/` are one reproducible lifecycle unit, but neither Git presence nor ignore status can prove that a future commit will contain both. An apply-time fact is exact about what this run changed and avoids turning read-only discovery/diff into a version-control warning. `info` rather than `warn` follows from the same Git-blindness: unable to tell a project that already versions both from one that ignores them, it fires on essentially every successful state-changing lifecycle operation, and a warning present on the whole happy path devalues the conditional warnings printed beside it. One exception, added 2026-08-06: when the project's own `.gitignore` carries a recognized rule hiding `.manteen/`, the advisory is raised to `warn` and names the failure it prevents. The check is one-directional — a match is evidence of a problem, no match is not evidence of correctness — so the advisory prints either way and nothing is gated on it. | Shelling out to Git makes a Git-independent Node tool depend on one VCS and still cannot prove a future commit; a plan diagnostic fires before apply knows whether state changed; `warn` for a standing storage fact trains readers to skip the severity that also carries `provider-missing` and `theme-conflict`. Also rejected: treating "cannot prove a guarantee" as "cannot help at all" — the original rejection of `.gitignore` reading conflated a partial parser's false PASSES (real, which is why this gates nothing) with its true POSITIVES (reliable, and the likeliest real-world way to break `update`); and a full pattern engine, which looks authoritative and makes every gap read as a verdict. |
| D40 | Post-update verification is opt-in, project-owned, and outside `apply()` and its journal. Ordered configured package scripts are resolved in plan, run fail-fast after every successful non-dry update (including a byte-identical update), and reported on `UpdateResult`, never `ApplyOutcome`. Planning hashes the whole root `package.json`; preflight checks that hash, post-apply orchestration revalidates the exact script definitions, snapshots the receipt, its ordinary destinations/bases and recorded current theme/styles, `manteen.json`, and `package.json`, then detects drift after each check. A failed check exits 1 but leaves the coherent update applied. Each check carries a wall-clock ceiling — `verification.timeoutMs`, five minutes by default, per check rather than per run — and a terminated check reports `timed-out`, never `script-failed`. | A consumer's own typecheck/test/build is the only honest way to move beyond “conflict-free text,” while arbitrary scripts can mutate files the Manteen journal does not own. Separating the completed transaction from verification preserves rollback truth and makes applied versus verified independent facts. Exact definitions prevent a dependency lifecycle script from substituting a command, and the bounded snapshot detects invalidated ownership state without claiming to sandbox project code. A ceiling is what makes "verification failed" reachable at all for a script that never returns, and per-check keeps an earlier script's duration from deciding whether a later one may finish — a per-run budget would make reordering the list change the verdict. | Running scripts inside the journal and claiming rollback; inferring scripts; accepting shell strings in config; executing a changed definition; persisting an environment-specific certificate in the receipt; describing a verification failure as an unapplied update; reporting a terminated check as `script-failed`, which sends the reader after a bug in their suite rather than a process that never finished; leaving the ceiling opt-in, which leaves the default path able to hang forever. |
| D41 | Ordinary update output remains exact source bytes. Line diff3 is always first. Only a remaining `.ts`/`.tsx` conflict may automatically enter the AST-assisted fallback shared by `diff` and `update`; there is no flag. The fallback parses read-only, accepts only disjoint changes to stable unique imports or exported top-level declarations, proves each whole side reconstructs byte-for-byte from exact source ranges, combines only non-overlapping ranges, and requires the result to parse. A ts-morph printer or `formatText()` path is rejected. Ambiguous mappings, including delete-versus-modify, preserve the original diff3 conflict. The implementation is public in client `0.5.0`. | A bounded printer probe found that formatting one 119-line TSX input changed 89 lines despite changing no program behavior; every observed change was whitespace, and the tested printer path also failed the exact CRLF/BOM boundary. The later read-only classifier used exact source offsets and emitted no merge bytes. Across two real upstream pairs plus nine controlled synthetic local sides, it found one distinct-import case that line diff3 reported as a conflict and produced zero false-independent results, while refusing same-key, rename/delete, parse, default, duplicate and cross-file ambiguity. The exact-splice evaluation covered all 25 eligible historical TS/TSX modifications across 17 paths, 76 disjoint and 24 same-key boundary cases, exact-byte/adversarial tests and runtime. The production implementation reproduces zero unsafe authorizations, rescues five constructed adjacent-anchor shapes, stays below its 250 ms p95 accepted-candidate ceiling, and passes source plan/diff plus built-Node CLI acceptance. The built merge chunk adds 9.95 kB raw / 2.67 kB gzip without a new parser dependency. The signed `0.5.0` tag, trusted publication, npm provenance and controlled exact-public-package accepted/refused receipt passed. | Making an AST printer or AST-first merger; treating the controlled corpus as observed consumer evidence; claiming semantic safety or a population conflict-rate improvement; weakening a refusal because an exported name merely survives a declaration-kind change; hiding fallback refusal instead of preserving the original diff3 conflict; calling an implementation public before its release receipts exist. See [`update-merge-handoff.md`](../handoffs/update-merge-handoff.md), the committed [`AST spike report`](../research/ast-assisted-merge-spike.md), the [`integration decision`](./ast-merge-integration-decision.md), and the [`0.5 release handoff`](../releases/v0.5-release-handoff.md). |
| D42 | Upstream file pruning is a separate `manteen remove --upstream-removed` transaction over ordinary receipt files. Before proving any candidate it resolves one fail-closed graph seeded by every receipt item id, including zero-file/theme/style-only items, plus the complete current transitive `registryDependencies` closure; any unavailable member blocks the whole command. A candidate exists only when the current item with the same canonical id omits the exact POSIX receipt destination and no other member of that bounded graph claims it. Discovery is `--dry-run` with no selection; a real run requires repeated exact `--file` selections. Present source is adapted whenever its raw-byte hash differs from the receipt's pristine `baseSha256`, and deleting it additionally requires `--discard-adapted`. Removal consumes only config/receipt validity, current item/ref fetch+wire resolution, ordinary target mapping/collision safety and removal-specific diagnostics; package-manager, dependency-range/Mantine, provider, CSS, theme-fold, styles-config, jsconfig, update-merge/base/conflict and verification gates are outside it. The command has no `--all`, `--yes`, `--force`, prompt, rename/content/AST inference, theme/style/dependency cleanup or item uninstall semantics. Source, derived base and receipt changes share a dedicated preflighted journal; the receipt is written last, and an emptied receipt item keeps its sticky `direct` state. Only a successful non-preview apply reports observed state-versioning change. | `update` deliberately retains files removed upstream because absence alone is not deletion authority. The receipt plus a completely resolved, explicitly bounded current graph can prove exact same-item omission and cross-owner safety without pretending to know the registry universe. The pristine hash distinguishes untouched upstream bytes from local adaptations even after an earlier clean merge changed `installedSha256`. Exact selection makes destructive intent reviewable and scriptable. Keeping unrelated install/update gates, theme/styles, dependencies, directories and the receipt item outside this operation prevents one file-pruning flag from becoming an implicit update or uninstall. | `update --prune`; deleting every `removed-upstream` diff row; resolving only selected owners; index/404 absence as proof; basename, source-path, similarity or AST rename inference; comparing current bytes only to `installedSha256`; bulk `--all`/`--yes`/`--force`; collecting a zero-file item; running normal `plan()`/`apply()`, which would also evaluate or mutate unrelated project concerns. See [`upstream-removal-handoff.md`](../handoffs/upstream-removal-handoff.md). |
| D43 | `list --query` ranks matches within each deterministic registry group by exact canonical id, exact name, exact title, title prefix, id/name substring, title substring, then description substring. Ties retain the pre-query item order. With no query, item order is unchanged. Machine rows expose all matching fields plus the winning rank. | The canonical index order is deterministic raw-name order, not authored relevance. Keeping it during search makes description-only noise outrank a direct identity match. A fixed rank vocabulary improves discovery while retaining reproducible output, stable group order, and an explicit tie-breaker an agent can explain. | A fuzzy or learned score whose weights and ties are invisible; globally mixing registries; reordering unqueried output; alphabetically breaking equal-rank ties. |
| D44 | Machine-mode init captures package-manager stdout and stderr instead of inheriting either stream. Successful captured output is discarded; failed captured output remains in the structured install failure. Human text mode keeps its existing inherited non-interactive output. | The ten-key envelope is useful only when stdout is exactly one JSON document. Dependency installation is part of the successful init path, so child progress output before the envelope is a contract violation rather than cosmetic noise. Capturing instead of dropping failure output keeps the refusal actionable. | Teaching parsers to skip leading lines; sending package-manager progress to machine stdout; suppressing failure evidence; making all human installs silent. |
| D45 | The client command envelope moves directly to schema version 2 and always includes top-level `actions`. Successful applicable mutating previews emit one exact argv rerun action coupled to their `planDigest`; discovery-only removal with no selection emits none. Add payloads include the exact dependency-manager command or `null`. There is no version-1 compatibility emission. | There are no full-time users whose parsers justify maintaining two plausible outer contracts. A first-class reviewed rerun removes error-prone flag reconstruction while remaining an argv array rather than executable shell text. Exact dependency preview closes the gap between reviewed package work and the subprocess Manteen will run. | A version negotiation layer; optional top-level actions; teaching callers how to rebuild argv; display-only dependency summaries. |
| D46 | `registry list/add/remove` and `verification show/set/clear` are bounded `manteen.json` editors. Mutations are dry-run/expected-plan coupled, revalidate the exact config preimage at write time, and replace only their owning top-level member. Registry replacement/removal fails closed against receipt ownership; the last registry cannot be removed. Verification values are discovered `package.json` script names, preserve authored order, and require at least one operation list. | Hand editing remained the only route for two frequent setup tasks and gave agents no plan, mutation truth, or ownership refusal. Whole-file serialization creates unrelated diffs and risks template disclosure. These commands can be safe precisely because their ownership is narrow and their inputs are structured. | A generic JSON patch command; whole-file reserialization; `--force` for receipt references; shell-string verification; inferred default checks. |
| D47 | Registry headers accepted by `registry add` must retain a literal `${VAR}` template. Machine output exposes header and parameter keys but never expanded values. | Authentication configuration must remain reviewable without allowing a command result or plan to become a secret transcript. Requiring templating also prevents an agent from persisting a copied bearer token directly through the convenience command. | Accepting literal header secrets; expanding templates for previews; omitting all source identity from the plan. |
| D45 | Existing-config migration distinguishes missing fields, explicit conflicts, and invalid shapes. An absent `@house` member in a valid registry map and an absent detected `theme` path are filled additively while preserving custom registries and unrelated keys. Missing ownership-selecting fields such as `aliases`, or an absent `tsconfig` when the detected application config is not the default, still refuse with an exact `configPatch`; explicit differing values remain non-forceable. | Absence is not an authored adaptation, and init already emits these canonical values for a new project. Narrow completion removes needless fixture-owner intervention without authorizing a general config rewrite. Destination aliases and non-default tsconfig selection remain reviewable because choosing them determines where future registry bytes land. | Describing absent values as explicit conflicts; replacing custom registry/theme values; completing every partial config indiscriminately; silently guessing source destinations. |
| D46 | A successful real `add` reports that registry installation does not establish application integration. Text mode writes one informational `application-integration` advisory to stderr; JSON mode carries the same text in `notes`. Dry runs, refusals, cancellations, and failed or rolled-back applies omit it. The packaged skill tells agents to inspect `info` metadata and edit consumer-owned application code only when the task asks to use the item. | Registry ownership ends at installed source, bases, receipt, dependencies, styles, and theme contribution. Route placement and UI composition are product decisions Manteen cannot infer, but an agent must not mistake a healthy receipt for a completed use-site request. A just-in-time note plus conditional guidance states both boundaries without forcing integration for install-only work. | Automatically editing `App.tsx` or guessing a route; claiming an item is rendered from receipt health; warning on previews or failed applies; requiring every installed primitive to appear immediately in an application screen. |
| D48 | Vite init establishes Manteen's `@` runtime mapping with an explicit absolute alias derived through `fileURLToPath(new URL(..., import.meta.url))`. It preserves authored resolver members, accepts only the exact static alias shape it can prove on a second run, and refuses a conflicting or dynamic `resolve.alias`. It never creates `resolve.tsconfigPaths`; an authored value remains outside init ownership. | A clean-room `create-vite@8.0.2` consumer resolved Vite 7.3.6, where the previously generated Vite 8-only `resolve.tsconfigPaths` member failed `tsc -b` after init reported `complete: true`. The explicit alias is supported by both Vite 7 and 8, works for Vite's `src` and React Router's `app` source roots, and keeps init's success independent of a moving framework capability. | Branching on an imprecise declared semver range; adding a resolver plugin dependency; retaining a native option that invalidates Vite 7; replacing an authored alias without proof. |
| D49 | Offline status classifies an unreadable receipt as invalid JSON, invalid schema, unsupported version, future version, or I/O failure. Each remains an exit-zero, non-mutating unhealthy assessment, carries a sanitized warning and manual recovery action, and exposes no receipt bytes. Recovery treats `manteen.lock.json` and `.manteen/bases/` as one trusted state boundary and never reconstructs ownership by guessing. | The parser already retained these distinctions, but status collapsed them to `state: unreadable` and the schema-v2 envelope discarded its inventory note. An agent could stop safely but could not select a safe recovery. I/O failure must also remain distinct: calling a receipt directory “schema invalid” sends the user toward the wrong bytes. | A generic unreadable flag with no reason; raw JSON/Ajv or filesystem error text in machine output; automatic deletion or receipt reconstruction; making an unhealthy assessment a command failure. |
| D50 | Vite init matches the generated application's quote and import-semicolon convention, and wraps its returned JSX through a bounded exact-text patch that adds one indentation level without formatting unrelated source. Existing refusal and idempotence boundaries remain unchanged. | The Vite adapter configured quote style but let ts-morph print semicolons onto only Manteen's imports and reprint an already-indented root beneath `MantineProvider`, producing a visibly mixed and deeply indented fresh scaffold. The transform owns only its imports and wrapper, so it can normalize those exact additions without claiming ownership of the complete file. | Running Prettier or `formatText()` over consumer source; accepting a functionally valid but structurally noisy default; weakening source-shape refusals to make a broader printer pass possible. |
...
The probe checkpoint in `../handoffs/w6-init-handoff.md` closed these choices before implementation:
...
| **Project verification — D40 shipped in `0.3.0`; agent-native execution later extended the transaction boundary** | update orchestration | D40's post-update model is the historical `0.3.0` through `0.5.0` contract recorded in `../handoffs/update-verification-handoff.md`. The agent-native contract later made checks operation-specific for add/update/remove and transaction-bound for Manteen-owned preimages; arbitrary verifier and dependency-manager side effects remain outside rollback claims. It still does not bundle a formatter or validator. | Verification is complete and public. Bundled formatting remains high-cost because it changes install weight and byte ownership. See `agent-native-build-plan.md` for the current boundary. |
...
| **Persisting the journal to disk** (surviving SIGKILL mid-write) | apply | The mutating window is a short in-memory write phase. The journal is best-effort convenience rather than crash durability; rollback failure returns the unrestored paths and directs recovery from version control or another trusted pre-run copy without assuming Git. | Low: write pre-images to a durable project-local recovery area before phase 3. |
