...
function manualInstruction(project: InitProjectSnapshot): InitInstruction {
  return {
    ...
    message:
      "Mount MantineProvider with the exported @/lib/theme, import @mantine/core/styles.css, and add ColorSchemeScript only when this framework renders the document on the server.",
  };
}
...
export function planShared(
  ...
  const results = [planConfig(project), planTheme(project), planTsconfig(project)];
